...
import { rememberContextEntryMessage } from "./context-refs";
import {
...
export function buildSessionContext(
	...
	const appendMessage = (entry: SessionEntry) => {
		if (entry.type === "message") {
			rememberContextEntryMessage(entry.message, entry.id);
			...
			rememberContextEntryMessage(message, entry.id);
	...
	if (compaction) {
		...
		rememberContextEntryMessage(compactionMessage, compaction.id);
...
export class SessionManager {
	...
	appendCompaction<T = unknown>(
		...
		rememberContextEntryMessage(
			createCompactionSummaryMessage(summary, tokensBefore, entry.timestamp, shortSummary),
			entry.id,
		);
		this.#appendEntry(entry);
		return entry.id;
	...
	appendCustomMessageEntry<T = unknown>(
		...
		rememberContextEntryMessage(
			createCustomMessage(customType, content, display, details, entry.timestamp, attribution),
			entry.id,
		);
		this.#appendEntry(entry);
		return entry.id;
	...
	branchWithSummary(branchFromId: string | null, summary: string, details?: unknown, fromExtension?: boolean): string {
		...
		rememberContextEntryMessage(
			createBranchSummaryMessage(summary, entry.fromId, entry.timestamp, details),
			entry.id,
		);
		this.#appendEntry(entry);
		return entry.id;
