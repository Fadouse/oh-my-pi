...
export class BranchSummaryMessageComponent extends Box {
	...
	constructor(
		...
		private readonly ctx?: Pick<
			...
			"agent" | "hideThinkingBlock" | "session" | "sessionManager" | "toolOutputExpanded" | "ui"
		>,
	) {
	...
	#renderCheckout(details: CheckoutSummaryDetails): void {
		...
		this.addChild(new Text(theme.fg("customMessageText", title), 1, 0));
		if (summary.nextStep) {
		...
		this.addChild(
			new Text(
				muted(
					this.#expanded
						? "Expanded checkout: summary plus original messages between the markers below"
						: "Checkout summary (ctrl+o to expand original messages)",
				),
				1,
				0,
			),
	...
	#addOriginalMessage(message: AgentMessage, pendingTools: Map<string, ToolExecutionHandle>): void {
		switch (message.role) {
			...
			case "assistant": {
				const assistantComponent = new AssistantMessageComponent(message, this.ctx?.hideThinkingBlock ?? false);
			...
			case "toolResult": {
				...
				if (component) {
					...
					this.addChild(new Text(theme.fg("toolOutput", `[tool result: ${message.toolName}]`), 1, 0));
				}
				break;
			}
			case "bashExecution": {
