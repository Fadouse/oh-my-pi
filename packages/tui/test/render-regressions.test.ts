...
function countExact(lines: string[], value: string): number {
	return lines.filter(line => line.trimEnd() === value).length;
}
...
describe("TUI terminal-state regressions", () => {
	describe("cursor + differential stability", () => {
		...
		it("can force redraw without deleting transcript scrollback", async () => {
			const term = new VirtualTerminal(40, 5);
			const tui = new TUI(term);
			const component = new MutableLinesComponent(rows("before-force-", 12));
			tui.addChild(component);

			try {
				tui.start();
				await settle(term);
				expect(term.getScrollBuffer().join("\n")).toContain("before-force-0");

				component.setLines(rows("after-force-", 12));
				tui.requestRender({ force: true, clearScrollback: false });
				await settle(term);

				const buffer = term.getScrollBuffer();
				expect(buffer.join("\n")).toContain("before-force-0");
				expect(buffer.join("\n")).toContain("after-force-");
				expect(countExact(buffer, "before-force-0")).toBe(1);

				component.setLines(rows("after-force-again-", 12));
				tui.requestRender({ force: true, clearScrollback: false });
				await settle(term);

				const secondBuffer = term.getScrollBuffer();
				expect(countExact(secondBuffer, "before-force-0")).toBe(1);
				expect(secondBuffer.join("\n")).toContain("after-force-again-");
			} finally {
				tui.stop();
			}
		});

		it("repaints shifted viewport when content expands above the visible slice", async () => {
			...
			try {
				tui.start();
				await settle(term);
				expect(visible(term)).toEqual(["tail-0", "tail-1", "tail-2", "tail-3", "tail-4", "tail-5"]);
				...
				expect(visible(term)).toEqual(["tail-0", "tail-1", "tail-2", "tail-3", "tail-4", "tail-5"]);
				expect(visible(term).join("\n")).not.toContain("checkout-collapsed");
			} finally {
				tui.stop();
			}
		});

		it("clears every viewport row during scrollback-preserving force redraw", async () => {
			const term = new VirtualTerminal(40, 6);
			const tui = new TUI(term);
			const component = new MutableLinesComponent(rows("long-preserve-", 20));
			tui.addChild(component);
			...
			try {
				...
				component.setLines(["short-preserve"]);
				tui.requestRender({ force: true, clearScrollback: false });
				await settle(term);

				const viewport = visible(term);
				expect(viewport[0]?.trim()).toBe("short-preserve");
				for (const line of viewport.slice(1)) {
					expect(line.trim()).toBe("");
				}
				expect(viewport.join("\n")).not.toContain("long-preserve-");
			} finally {
				tui.stop();
			}
		});

		it("keeps boolean force redraw behavior clearing scrollback", async () => {
			const term = new VirtualTerminal(40, 5);
			const tui = new TUI(term);
			const component = new MutableLinesComponent(rows("before-clear-", 12));
			tui.addChild(component);

			try {
				tui.start();
				await settle(term);
				expect(term.getScrollBuffer().join("\n")).toContain("before-clear-0");

				component.setLines(rows("after-clear-", 12));
				tui.requestRender(true);
				await settle(term);

				const buffer = term.getScrollBuffer().join("\n");
				expect(buffer).not.toContain("before-clear-0");
				expect(buffer).toContain("after-clear-");
			} finally {
				tui.stop();
			}
		});
